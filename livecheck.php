...
while($data = mysql_fetch_array($result)){
	...
	if($data['present']=='yes'){
		...
		echo "<td bgcolor='green'>".$data['present']."</td>";
		echo "</tr>";
	} else if($data['present']=='no'){
		if($data['st_stream']=='1'){
			$data['st_stream']='Non-Medical';
		} else if($data['st_stream']=='2'){
			$data['st_stream']='Commerce';
		...
		echo "<td>".$data['st_father']."</td>";
		echo "<td>".$data['st_mobile']."</td>";
		echo "<td><textarea id='st_address_".$data['st_id']."'>".$data['st_address']."</textarea></td>";
		echo "<td>".$data['st_email']."</td>";
		echo "<td>".$data['st_class']."</td>";
		echo "<td>".$data['st_stream']."</td>";
		echo "<td>".$data['st_school']."</td>";
		...
		echo "<td>".$data['st_activity']."</td>";
		?>
		<td> <button onclick="addPresentStatus(<?php echo $data['st_id']; ?>)"> No </button> </td>
		<?php
		echo "</tr>";
	}
}
